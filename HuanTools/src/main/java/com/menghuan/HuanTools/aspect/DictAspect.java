package com.menghuan.HuanTools.aspect;
...
import com.menghuan.HuanTools.aspect.dict.CommonAPI;
import com.menghuan.HuanTools.aspect.dict.CustConvertUtils;
import com.menghuan.HuanTools.aspect.dict.Dict;
import com.menghuan.HuanTools.aspect.dict.DictModel;
...
public class DictAspect {
    ...
    @Pointcut("execution(public * org.jeecg.modules..*.*Controller.*(..)) "
        + "|| execution(public * org.df..*.*Controller.*(..))"
    ...
    /**
     * 本方法针对返回对象为 Result 的 IPage 的分页列表数据进行动态字典注入 1、支持 IPage、List、Po - 250306 字典注入实现 通过对实体类添加注解@dict
     * 来标识需要的字典内容,字典分为单字典code即可 ，table字典 code table text配合使用与原来jeecg的用法相同 示例为SysUser 字段为sex 添加了注解@Dict(dicCode = "sex")
     * 会在字典服务立马查出来对应的text 然后在请求list的时候将这个字典text，已字段名称加_dictText形式返回到前端 例输入当前返回值的就会多出一个sex_dictText字段 { sex:1,
     * sex_dictText:"男" } 前端直接取值 sex_dictText 在 table 里面无需再进行前端的字典转换了 customRender:function (text) { if(text==1){ return
     * "男"; }else if(text==2){ return "女"; }else{ return text; } } 目前 vue 是这么进行字典渲染到 table 上的多了就很麻烦了，这个直接在服务端渲染完成前端可以直接用
     * 
    ...
    private Object parseDictText(Object result) {
        ...
        if (result instanceof Result) {
            ...
            if (((Result)result).getResult() instanceof Object) {
                // 取出结果集
                List<Object> records = Arrays.asList(((Result)result).getResult());
                Boolean hasDict = checkHasDict(records);
                if (!hasDict) {
                    return result;
                }
