...
public class containsTest {
    ...
    public void sss() {
        ...
        List<Integer> removeList = oneList.stream().filter(a -> !filterList.contains(a.getId()))
            .map(IotStructure::getId).collect(Collectors.toList());
        ...
        List<Integer> finalCollect = structureList.stream().filter(a -> !removeList.contains(a.getId()))
            .map(IotStructure::getId).collect(Collectors.toList());
